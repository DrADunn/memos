    ...
    const cellContent = (
      <div
        className={cn(
          ...
          dayInfo.isCurrentMonth && getCellOpacity(maxCount ? count / maxCount : 0),
...
export const ActivityCalendar = memo(
  observer((props: ActivityCalendarProps) => {
    ...
    const weekStartDayOffset =
      workspaceStore.state.generalSetting.weekStartDayOffset;

    // 允许父组件传入筛选后的 memos（不改全局类型，直接宽松处理）
    const memos = (props as any).memos as any[] | undefined;
    ...
    const { days, weekDays, maxCount } = useMemo(() => {
      ...
      const firstDay =
        (((new Date(yearValue, monthValue, 1).getDay() - weekStartDayOffset) %
          7) +
          7) %
        7;
      const lastDay =
        new Date(yearValue, monthValue, dayInMonth).getDay() -
        weekStartDayOffset;
      ...
      for (let i = 1; i <= dayInMonth; i++) {
        const date = dayjs(`${yearValue}-${monthValue + 1}-${i}`).format(
          "YYYY-MM-DD",
        );
